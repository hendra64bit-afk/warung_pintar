...
const getAiClient = () => {
  ...
  if (!apiKey) {
    throw new Error("GEMINI_API_KEY tidak ditemukan di environment. Pastikan kunci telah diatur pada pengaturan Vercel.");
  }
...
/**
 * Endpoint Analisis Cerdas berbasis Gemini untuk Vercel Serverless
 */
app.post("/api/analyze", async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn("Kunci API Gemini tidak dikonfigurasi, mengaktifkan mesin fallback lokal:", apiError.message);
    }
...
`;
      ...
      for (const model of models) {
        ...
        console.log(`[Vercel Serverless Gemini] Mencoba pemanggilan konten menggunakan model: ${model}`);

        for (let attempt = 1; attempt <= maxRetriesPerModel; attempt++) {
          try {
            response = await ai.models.generateContent(apiParams);
            if (response && response.text) {
              console.log(`[Vercel Serverless Gemini] Berhasil diselesaikan menggunakan model ${model} pada percobaan #${attempt}.`);
              break;
            }
          } catch (err: any) {
            lastError = err;
            ...
            console.warn(`[Vercel Serverless Gemini] Model ${model} gagal pada percobaan #${attempt}:`, message);
            ...
            if (isServiceUnavailable && attempt < maxRetriesPerModel) {
              ...
              console.log(`[Vercel Serverless Gemini] Menunggu ${delay}ms sebelum mencoba kembali...`);
    ...
    // FALLBACK ENGINE: Jika seluruh pemanggilan model Gemini gagal (karena 503, overload, dsb.) atau Kunci API kosong
    console.warn("[Vercel Serverless Gemini Fallback] Mengaktifkan mesin kalkulasi analitik retail lokal...");
    ...
    const insights = [
      "Koneksi server utama atau kunci API sedang memerlukan konfigurasi ulang. Menampilkan evaluasi analitik lokal berbasis kalkulasi keuangan dan persediaan riil toko.",
      ...
      highProfitItems.length > 0 
        ? `Produk "${highProfitItems[0].name}" menyumbangkan perolehan margin laba kotor tertinggi, sebesar ${highProfitItems[0].profit.toLocaleString('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0, maximumFractionDigits: 0 })}.`
        : "Belum ada akumulasi laba kotor signifikan karena aliran transaksi penjualan masih minim."
    ];

    const recommendations = [
      "Maksimalkan penawaran bundling silang (cross-selling) dengan mencampurkan sediaan terlaris dan produk lambat laku pemasaran.",
      "Segera lakukan pengisian persediaan barang kritis yang tercantum pada tabel 'Alert Deteksi Stok Kritis' demi mengamankan kepuasan pelayanan pelanggan.",
      "Tetapkan strategi harga jual minimal 15% - 20% di atas HPP produk agar kesehatan arus kas warung Anda tetap ideal."
    ];

    return res.json({
      summary: "Pusat server AI Gemini sedang mengalami kepadatan akses (Spikes/503) di wilayah serverless Lambda atau kunci API belum diatur di dashboard Vercel Anda. Berkat sistem ketahanan WarungPintar, kami menyajikan evaluasi analitik lokal cerdas secara tangguh.",
      criticalStockAlerts,
      profitAnalysis: {
        insights,
        recommendations
